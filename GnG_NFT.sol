...
interface ICallistoNFT {
    ...
    function setTokenProperty(uint256 _tokenId, uint256 _propertyId, string calldata _content) external;
    function addProperties(uint256 _tokenId, uint256 _propertiesCount) external;
...
abstract contract CallistoNFT is ICallistoNFT, Ownable {
    ...
    function setTokenProperty(uint256 _tokenId, uint256 _propertyId, string calldata _content) public override onlyOwner
    {
        require(msg.sender == owner(), "NFT: only owner can change content of properties with index 1+");
        _tokenProperties[_tokenId].properties[_propertyId] = _content;
    }

    function addProperties(uint256 _tokenId, uint256 _propertiesCount) public override onlyOwner
    {
        for (; _propertiesCount > 0; _propertiesCount--)
        {
